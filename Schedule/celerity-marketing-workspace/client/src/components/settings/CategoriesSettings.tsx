...
export function CategoriesSettings() {
  ...
  const [draft, setDraft] = useState({ nameEn: "", nameEs: "" });
  const [pendingDelete, setPendingDelete] = useState<Category | null>(null);

  const handleAdd = async () => {
    if (!draft.nameEn.trim() || !draft.nameEs.trim()) return;
    await createCategory({ nameEn: draft.nameEn.trim(), nameEs: draft.nameEs.trim() });
    setDraft({ nameEn: "", nameEs: "" });
    setAdding(false);
  };
    ...
    <div className="space-y-4">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-surface text-left text-xs font-semibold uppercase tracking-wide text-muted-foreground">
              <th className="px-4 py-3">{t("settings.categories.nameEn")}</th>
              <th className="px-4 py-3">{t("settings.categories.nameEs")}</th>
              ...
              <tr key={cat.id} className="border-b border-border last:border-b-0">
                <td className="px-4 py-2.5">
                  <Input
                    ...
                    onBlur={(e) => e.target.value.trim() && e.target.value !== cat.nameEn && updateCategory(cat.id, { nameEn: e.target.value.trim() })}
                ...
                <td className="px-4 py-2.5">
                  <Input
                    ...
                    onBlur={(e) => e.target.value.trim() && e.target.value !== cat.nameEs && updateCategory(cat.id, { nameEs: e.target.value.trim() })}
                    className="h-8 border-transparent bg-transparent px-1 hover:border-input focus:border-brand-700 focus:bg-white"
                  />
                </td>
                <td className="px-4 py-2.5">
            ...
            {adding && (
              <tr className="border-b border-border bg-surface/50">
                <td className="px-4 py-2.5">
                  <Input
                    autoFocus
                    placeholder={t("settings.categories.nameEn")}
                    value={draft.nameEn}
                ...
                <td className="px-4 py-2.5">
                  <Input
                    placeholder={t("settings.categories.nameEs")}
                    value={draft.nameEs}
      ...
      {!adding && (
        <Button variant="secondary" size="sm" onClick={() => setAdding(true)}>
          <Plus className="h-3.5 w-3.5" /> {t("settings.categories.add")}
        </Button>
      )}
